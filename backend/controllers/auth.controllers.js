...
export const signUp = async (req, res) => {
  try {
    ...
    res.cookie("token", token, {
      httpOnly: true,
      maxAge: 10 * 365 * 24 * 60 * 60 * 1000, //milisec
      secure: true,
      sameSite: "None",
    });
...
export const signIn = async (req, res) => {
  try {
    ...
    const token = await genToken(findByUserName._id);

    res.cookie("token", token, {
      httpOnly: true,
      maxAge: 10 * 365 * 24 * 60 * 60 * 1000, //millisec
      secure: true,
      sameSite: "None",
    });

    return res.status(200).json(findByUserName);
  } catch (error) {
